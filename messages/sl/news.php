<?php
/**
 * Created by PhpStorm.
 * User: Milic_lenovo
 * Date: 12.7.2017
 * Time: 21:03
 *
 *  Slovenian translations for news
 */


return [

    'News'=>'Novice',
    'News and articles'=>'Novice in članki',

    // artcle titles
    'Why walking your dog is great exercise?'=>'Zakaj je sprehajanje psov dobra rekreacija?',
    'Pets can benefit from spending time outside'=>'Prednosti preživljanja časa s hišnimi ljbljenčki v naravi:',
    'Pet walkers to be more in demand than teachers in next decade'=>'Povpraševanje po sprehajalcih živali, večje kot pri učiteljih',

    //intro articles
    'Having trouble sticking to an exercise program? Research shows that dogs are actually Nature’s perfect personal trainers—loyal, hardworking, energetic and enthusiastic. And, unlike your friends, who may skip an exercise session because of appointments, extra chores or bad weather, dogs never give you an excuse to forego exercising'
    =>
        'Ali imate težave z izvajanjem vadbenega programa? Raziskave kažejo, da so psi pravzaprav popolni osebni trenerji. So zvesti, trdo delovni, energičeni in navdušeni. Za razliko od vaših prijateljev, ki lahko preskočijo vadbo zaradi sestankov, dodatnih opravil ali slabega vremena, vam psi nikoli ne dajejo izgovora, da se izognete vadbi.',

    'Most pets already spend a little time outside everyday, mainly when going out to go to the bathroom. But many owners don\'t fully appreciate the added benefits from allowing their pets to spend more significant time outdoors. The following is just a few of the benefits for your pet from being outside:'
    =>
        'Večina živali že preživi nekaj časa zunaj vsak dan, predvsem takrat, da gredo na stranišče. Mnogi lastniki ne cenijo dodane koristi. V nadaljevanju je le nekaj prednosti za vašega ljubljenčka od tega, da bela zunaj:',


    'Memo to job-seekers: You\'ve probably got more of a chance walking dogs for a living than teaching kids in the coming decade\'s labor market. That\'s one of the implicit messages in a new report from the New York-based Conference Board on the changing composition of consumer demand the main driver of the economy over the next 10 years'
    =>
        'Zanimivost za iskalce zaposlitve: verjetno boste imeli več priložnosti, da opravljaste poklic sprehajalca, kot učitelja v prihodnjem desetletju. To je eno od implicitnih sporočil v novem poročilu konferenčnega odbora v New Yorku o spreminjajoči se sestavi povpraševanja potrošnikov - glavnem gonilu gospodarstva - v naslednjih 10 letih.',

    'Sitters in demand'=>'Povpraševanje po sprehajalcih',
    'Walking the dog'=>'Sprehajanje psa',
    'New job opportunities'=>'Nove karijerne priložosti',
    'Spending time outside'=>'Preživljanje prostega časa na prostem',


    //članki

    // first news

    'Having trouble sticking to an exercise program? Research shows that dogs are actually Nature’s perfect personal trainers—loyal, hardworking, energetic and enthusiastic. And, unlike your friends, who may skip an exercise session because of appointments, extra chores or bad weather, dogs never give you an excuse to forego exercising.'
        =>
            'Ali imate težave z izvajanjem vadbenega programa? Raziskave kažejo, da so psi pravzaprav popolni osebni trenerji. So zvesti, trdo delovni, energičeni in navdušeni. Za razliko od vaših prijateljev, ki lahko preskočijo vadbo zaradi sestankov, dodatnih opravil ali slabega vremena, vam psi nikoli ne dajejo izgovora, da se izognete vadbi.',


    'In 2008, the U.S. Department of Labor reported that only 16 percent of Americans ages 15 and older exercised at all on an average day! This is where your canine personal trainer can help. A survey of dog owners, conducted at the University of Western Australia and published in Health Promotion Journal of Australia in August 2008, revealed that dogs are great motivators for walking because they:'
        =>
            'Leta 2008 je ameriško ministrstvo za delo poročalo, da je le 16 odstotkov Američanov, starih 15 let in več, izvajalo fizične aktivnosti dnevno! Tu vam lahko pomagajo hišni ljubljenčki. Anketa lastnikov psov, ki je bila izvedena na Univerzi v zahodni Avstraliji in ki je bila avgusta 2008 objavljena v reviji za promocijo zdravja Avstralije, je pokazala, da so psi odlični motivatorji za hojo, ker:',

    'Provide a strong motivation to maintain a program'=>'Zagotovite močno motivacijo za vzdrževanje programa',
    'Are good walking companions'=>'So dobri spremljevalci',
    'Provide good social support when exercising'=>'Zagotovite dobro družbo pri vadbi',

    'What are the benefits of regular exercise? Dr. Joanna Kruk reviewed medical literature describing the health benefits of exercise. Her research showed that the risk of developing a number of serious health problems is reduced by physical activity and exercise:'
        =>
            'Kakšne so prednosti redne vadbe? Dr. Joanna Kruk je pregledala medicinsko literaturo, ki opisuje zdravstvene koristi vadbe. Njena raziskava je pokazala, da se tveganje za nastanek številnih resnih zdravstvenih težav zmanjša s telesno aktivnostjo:',

    'Breast cancer risk reduced by 75 percent'=>'Tveganje raka dojk se je zmanjšalo za 75 odstotkov',
    'Heart disease risk decreased by 49 percent'=>'Tveganje za bolezni srca se je zmanjšalo za 49 odstotkov',
    'Diabetes risk lowered by 35 percent'=>'Tveganje sladkorne bolezni se je znižalo za 35 odstotkov',
    'Colon cancer risk decreased by 22 percent'=>'Tveganje za raka debelega črevesa se je zmanjšalo za 22 odstotkov',

    'Researchers at the University of Western Australia found that seven in every 10 adult dog owners achieved 150 minutes of physical exercise per week, compared with only four in every 10 non-owners. Among new dog owners monitored for one year, recreational walking increased by an average of 48 minutes per week. And, among folks like you who read dog magazines, six in every 10 walked their dogs every day.'
        =>
            'Raziskovalci na Univerzi v Zahodni Avstraliji so ugotovili, da je sedem na vsakih 10 odraslih lastnikov psov doseglo 150 minut telesne vadbe na teden, v primerjavi s samo štirimi na vsakih 10 ne-lastnikov. Med novimi lastniki psov, ki so bili spremljali eno leto, se je rekreativna hoja povečala v povprečju za 48 minut na teden.',

    'Is dog walking really effective exercise? Many people are become interested in exercise to help lose excess weight. Obesity is a global epidemic, affecting about one in every three to four adults in the United States and Europe. Dog ownership and obesity were evaluated in Seattle, Wash., and Baltimore, Md., in a study published in the journal Preventive Medicine in September 2008. Dog owners who reported walking their dogs were almost 25 percent less likely to be obese than people without dogs. Researchers in the April 2008 issue of Health Promotion Journal of Australia reported that having a dog in the house reduced the risk of childhood obesity by half!'
        =>
            'Ali je sprehajanje psov res tako učinkovito? Mnogi ljudje postajajo zainteresirani za vadbo, da si pomagajo izgubiti prekomerno težo. Debelost je globalna epidemija, ki prizadene približno vsaj enega od treh do štirih odraslih v Združenih državah in Evropi. Lastniki psov, so imli 25% manj možnosti, da bi bili debeli kot ljudje brez psov. Raziskovalci so v aprilu 2008 poročali, lastništvo psa zmanjša tveganje za debelost otroka za polovico!',

    'How much exercise is enough? According to the World Health Organization, adequate exercise to promote good health includes:'
        =>
            'Koliko vadb je dovolj? Po navedbah Svetovne zdravstvene organizacije je ustrezno spodbujanje dobrega zdravja potrebno vključevati:',

    '60 minutes of moderate to vigorous activity daily for children 5 to 17 years old' =>'60 minut zmerne do močne aktivnosti dnevno za otroke od 5 do 17 let',
    '30 minutes of moderate-intensity exercise five days per week for adults 18 to 65 years old, plus strengthening exercises two days per week' =>'30 minut zmerne intenzivnosti vadbe pet dni na teden za odrasle od 18 do 65 let',
    '30 minutes of moderate-intensity exercise five days per week, with modifications as needed in seniors over 65 years old, plus flexibility and balance exercises' =>'30 minut z zmerno intenzivnostjo pet dni na teden, s potrebnimi spremembami pri starejših nad 65 let',


    'Plan for success. It’s easy to forget about healthy walking plans, so set the stage for a successful program:'=>'Načrt za uspeh. Zelo enostavno je pozabiti na načrte za zdravo hojo, zato si postavite izhodišče za uspešen program:',

    'If dog walking is “scheduled” into each day, you’ll feel more responsible for sticking with your program. Plus, your dog will also get used to the routine and remind you when “it’s time!”'
        =>
            'Če je hoja psa "načrtovana"  vsak dan, se boste počutili bolj odgovorni za ohranjanje vašega programa. Poleg tega se bo vaš pes navadil na rutino in vas spomnil, kdaj je "čas!"',
    'Track your progress'=>'Sledite svojemu napredku',
    'Post a calendar on the refrigerator and add a sticker for each 10 minutes of walking you do each day. This will reinforce your good behavior and make you pause before opening the door to grab a calorie-laden snack!'
        =>
            'Koledar objavite na hladilniku in dodajte nalepko za vsakih 10 minut hoje, ki jo naredite vsak dan.',
    'So, grab a leash, whistle up the pup, and go for a walk—today and every day! Dog walking is a great way to jumpstart a healthy lifestyle program.'
        =>
            'Torej, zgrabite povodec in pojdite na sprehod - danes in vsak dan! Sprehajanje je odličen način za začetek zdravega načina življenja.',

    'Establish a walking schedule; plan to walk 30 minutes total each day. This might include a 10-minute neighborhood walk in the morning and a 20-minute romp at the dog park after work. Or maybe three 10-minute walks or one 30-minute walk fit in better with your day.'
    =>
        'Določite urnik hoje; Planirajte vsak dan hoditi po 30 minut. To bi lahko vključevalo 10-minutni sprehod po sosedski zjutraj in 20-minutno zabavo v parku popoldne.',



    // second news

    'Memo to job-seekers: You\'ve probably got more of a chance walking dogs for a living than teaching kids in the coming decade\'s labor market. That\'s one of the implicit messages in a new report from the New York-based Conference Board on the changing composition of consumer demand -- the main driver of the economy -- over the next 10 years.'
        =>
            'Zanimivost za iskalce zaposlitve: verjetno boste imeli več priložnosti, da opravljaste poklic sprehajalca, kot učitelja v prihodnjem desetletju. To je eno od implicitnih sporočil v novem poročilu konferenčnega odbora v New Yorku o spreminjajoči se sestavi povpraševanja potrošnikov - glavnem gonilu gospodarstva - v naslednjih 10 letih.',

    'Its focus is on demographics, both the well-known aging of the Baby Boom generation and the less-publicized baby bust that began during the Great Recession as fertility rates dropped. Thus the choice of profession suggested by the business membership and research association\'s report. Spending on pets is forecast to rise strongly as boomers -- perhaps pining for children who have flown the coop -- shower their attention and money on new-found furry friends. Outlays on education will lag, though, as the potential student population comprising five- to 24-year-olds grows very slowly due to the downsized, post-Millennial Generation Z'
        =>
            'Njen poudarek je na demografiji, tako dobrodelni generaciji Baby Boom generacije kot tudi manj populariziranem rojstvu dojenčkov, ki se je začelo med veliko recesijo, saj so se stopnje rodnosti zmanjšale. Tako je izbira poklica, ki jo predlaga poslovno članstvo in poročilo raziskovalnega združenja. Predvideva se, da bo poraba za hišne ljubljence močno narasla, saj boomers - prenašajo svojo pozornost in denar na nove kosmate prijatelje. Izdatki za izobraževanje pa bodo zaostali, saj potencialna študentska populacija, ki jo sestavlja 5 do 24 let, raste zelo počasi zaradi zmanjšane proizvodnje po tisočletni generaciji Z',

    'A word about the chart. The study projects spending increases based purely on demographics, including the growth of the population. The figures don\'t factor in the impact of rising wages and wealth over the period. As such, the projected 8.1 percent upturn in total household expenditures from 2015 to 2025 understates the increase that will actually happen. It\'s no surprise to see health-care expenditures surging as the number of Americans between the ages of 70 and 84 spikes by 50 percent.'
    =>
        'Beseda o grafikonu. Študija predvideva povečanje porabe zgolj na demografskih podatkih, vključno z rastjo prebivalstva. Številke ne vplivajo na vpliv naraščajočih plač in premoženja v tem obdobju. Kot taka, predvideni 8,1-odstotni upad v skupnih izdatkih gospodinjstev od leta 2015 do leta 2025 podcenjuje povečanje, ki se bo dejansko zgodilo. Ni presenečenje, da se zdravstveni stroški povečujejo kot število Američanov v starosti od 70 do 84 ur za 50 odstotkov.',

    'But spending on books, newspapers and other reading materials will also boom as the growing ranks of retirees -- they\'re increasing by about 1.2 million a year, according to the report -- have more time to themselves. They\'ll also sell out more on home repairs as they become less willing or able to do them on their own,  the report says. The aging of the overall population results in smaller increases in spending on a variety of other goods and services -- restaurant meals and apparel, to name just two -- that are more popular with younger Americans. There\'s "reshuffling of the overall consumer mindset" coming in the next decade.'
    =>
        'Toda poraba za knjige, časopise in druga bralna gradiva bo tudi naraščala kot narašča število upokojenceev - povečujejo se za približno 1,2 milijona letno, v skladu s poročilom - imajo več časa za sebe. Prav tako bodo zapravili več za popravila okrog hiše, saj postanejo manj pripravljeni ali sposobni, da bi jih opravili sami, poročilo pravi. Staranje celotnega prebivalstva povzroča manjše povečanje izdatkov za različne druge izdelke in storitve - restavracijske jedi in oblačila, ki jih imenujemo le dve - ki sta bolj priljubljena pri mlajših Američanih. Obstaja "preusmeritev celotne potrošniške miselnosti", ki prihajajo v naslednjem desetletju.',





    //third news

    'Exercise...Very few pets get enough exercise, and for most dogs it is extremely difficult to be properly exercised indoors. Most dogs and many cats will greatly benefit from a couple of daily walks or some play time in the yard chasing a ball or running.'
    =>
        'Vaja ... zelo malo živali dobili dovolj vadbe, in za večino psov je zelo težko, da se ta izvaja zaprtih prostorih.',

    'Weight loss and control...Dogs that spend time outdoors have an easier time losing weight and keeping their weight down. Many dogs gain weight because of boredom and lack of activity. Spending time outside eliminates both of these contributors to weight gain.'
        =>
            'Izguba in nadzor teže ... Psi ki preživijo več časa zunaj imajo več možnosti za izgubo odvečne telesne teže. Mnogi psi pridobili težo zaradi dolgočasja in pomanjkanje aktivnosti.',

    'Relieve Boredom...Like people, dogs and cats get bored. In fact, boredom is one of the leading causes of behavior problems, including chewing and excessive barking. The outdoors provides a different and interesting environment that will help keep your pet\'s mind active and alert while curbing behavior you would rather not see.'
    =>
        'Pregnajte dolgčas ... Tako kot ljudje, psi in mačke se dolgoasijo. Dejstvo je, dolgčas, je eden od glavnih vzrokov za vedenjske težave, vključno z žvečenjem in pretiranim lajanjem. Čaš zunaj zagotavlja drugačno in zanimivo okolje, ki bo pomagala ohraniti vašega ljubljenčka aktivnega.',


    'Many dogs that aren\'t used to spending time outdoors are often at a loss initially to know what to do. They will often stand at the door and beg to come in because spending time outdoors may be an unusual experience for them. A few tips when getting your pet to spend time outdoors include:'
    =>
        'Mnogi psi, ki se ne preživijo zadostno časa na prostem, se soočajo z izgubo nagona. Pogosteje stojijo pri vratih in prosijo, da pridejo noter v hišo, zato ker je preživljanje časa zunaj zanje nenavadna izkušnja. Nekaj nasvetov kaj početu zunaj z novim ljubljenčkom:',


    'Spend time with them outside. Go for a walk and use a retractable leash to help them get more exercise.'
    =>
        'Preživite čas skupni čas zunaj. Pojdite na sprehod in uporabljte zložljiv povodec.',


    'Play a game. Throw a flying disc on land or in the water. This is a great way to exercise and relieve boredom.'
    =>
        'Posvetite jim pozornost. Frizbi je odličen način za rekreiranje in lajšanje dolgčasa.',


    'Provide stimulation in the form of toys or chews. Many dogs love to go outside and chew on a rawhide or similar bone, or a rubber chew toy. While they may not be exercising while they chew, they get all the other benefits of being outdoors.'
    =>
        'Poskrbite za stimulacijo v obliki igrač ali nagrade. Mnogi psi imajo radi, da gredo ven in žvečijo kosti, žvečilne igrače  ali podobno.',


    'Spending time outdoors is a very normal and important part of a pet\'s life. As long as they are safely contained, supervised, and protected from the elements, outside time is one of the best health benefits you can give your pet. And don\'t forget, your pet should always have a current nametag attached to his collar whenever he goes outside.'
    =>
        'Poraba časa na prostem je naravna in zelo pomembena za zdravo življenje ljubljenčka. Dokler so varni, nadzorovani in zaščiteni pred zunajimi elementi, to je eden izmed najboljša zdravstvna korist, za vaše hišne ljubljenčke. Ne pozabite, je da dobro imeti ovratnico z imenom živali in lastnika, ko greste na sprehod.',


// NOT TRANSLATED YET

    'Petwalkers is a  website for finding a perfect sitter for your little friend. Thus dogs and cats are most popular pets, you can use it for any other, from turtle to elephant. We encourage o review your sitters so people can build an amazing community and have better experience using this platform!'

        =>
        'Petwalkers je spletna aplikacija na kateri boste poiskali najboljšega sprehajalca za vašega kosmatinca. Čeprav so najštevilčnejši psi in mačke kot hišne živali aplikacijo lahko uporabljaste tudi za druge hišne ljubljenčke. Po opravljeni storitvi spodbujamo ocenjevanje sprehajalcev, saj tako lahko izboljšamo izkušnjo in skupnost na spletni strani.',

    'I realized that in Slovenia we do not have a simple way to coordinate the walking and sitting of pets, especially dogs and cats. With this I got the idea to develop web application and soon it turned out that I had enough knowledge to complete the project. The application would serve as a connecting  bridge between people who have more free time, who love animals and people who are employed or on holiday, and do not have time to walk or take cate of pets.'
        =>
            'Spoznal sem, da v Sloveniji nimamo preprostega načina za koordinacijo sprehajanja in varstva hišnih ljubljenčkov predvsem psov in mačk. S tem sem dobil idejo za razvoj aplikacije in kmalu se je izkazalo, da imam dovolj znanja za izpeljavo projekta. Aplikacija bi služila kot povezovalna mreža med ljudmi, ki imajo več prostega časa, ter imajo radi živali, in ljudmi ki so zaposleni oziroma na dopustu, ter nimajo časa za sprehode ali varstvo hišnih ljubljenčkov.',

    'For any questions, comments and problems with the website are available to you by e-mail:'
        =>
            'Za vsa morebitna vprašanja, pripombe in težave s spletno stranjo smo vam na voljo po elektronski pošti:',

    ''
    =>
        '',

    ''
    =>
        '',

    ''
    =>
        '',

    ''
    =>
        '',
];